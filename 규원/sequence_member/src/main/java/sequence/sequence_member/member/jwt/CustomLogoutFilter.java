...
public class CustomLogoutFilter extends GenericFilterBean {
    ...
    private void doFilter(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws IOException, ServletException {
        // 🔥 Fix: Ensure we only process logout requests
        if (!"/api/logout".equals(request.getRequestURI()) || !"POST".equals(request.getMethod())) {
            filterChain.doFilter(request, response);
            return;
        }

        // 🔥 Fix: Avoid NullPointerException when getting cookies
        String refresh = null;
        Cookie[] cookies = request.getCookies();
        if (cookies != null) {
            for (Cookie cookie : cookies) {
        ...
        // 🔥 Fix: If no refresh token is found, return a proper error
        if (refresh == null) {
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
        ...
        // 🔥 Fix: Remove refresh token and update the cookie properly
        refreshRepository.deleteByRefresh(refresh);
